...
/**
 * Checks whether an application is the old hardcoded dummy placeholder
 */
export function isDummyMockApplication(app: Partial<SellerKybApplication>): boolean {
  if (!app) return true;
  const bName = (app.businessName || "").toLowerCase();
  const tName = (app.tradeName || "").toLowerCase();
  const addr = (app.warehouseAddress || "").toLowerCase();
  const gstin = (app.gstin || "").toUpperCase();

  // Flag known dummy placeholder patterns
  if (
    bName.includes("bhasker fashions private limited") ||
    tName === "bhasker fashions" ||
    addr.includes("plot 42, kiadb") ||
...
export async function fetchGenuineKybApplications(): Promise<SellerKybApplication[]> {
  ...
  // 3. Proactively sync genuine local applications to the server API so they are permanently on disk
