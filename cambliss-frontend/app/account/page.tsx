...
export default function AccountHubPage() {
  ...
  const rmaReturns: any[] = [];

  const navigationTabs = [
    { id: "orders", label: "📦 My Orders", badge: orders.length > 0 ? String(orders.length) : undefined },
    { id: "wishlist", label: "❤️ Wishlist", badge: wishlistItems.length > 0 ? String(wishlistItems.length) : undefined },
    { id: "addresses", label: "📍 Delivery Addresses", badge: "1" },
    { id: "profile", label: "👤 Profile & B2B GSTIN" },
    { id: "reviews", label: "⭐ My Reviews & Ratings" },
    { id: "returns", label: "🔄 Returns & RMA", badge: rmaReturns.length > 0 ? String(rmaReturns.length) : undefined },
          ...
          <div className="lg:col-span-9 space-y-6">
            ...
            {activeTab === "orders" && (
              <div className="rounded-[8px] border border-slate-200 bg-white p-5 sm:p-6 space-y-6 shadow-2xs">
                ...
                {orders.length === 0 ? (
                  <div className="p-8 text-center border border-dashed border-slate-200 rounded-[6px] bg-slate-50 space-y-3">
                    <span className="text-3xl">📦</span>
                    <h4 className="font-bold text-slate-800 text-sm">No orders placed yet</h4>
                    <p className="text-xs text-slate-500 max-w-sm mx-auto">
                      Once you complete a purchase, your packages and 6-stage telemetry tracking will be visible here.
                    </p>
                    <div className="pt-1">
                      <Link
                        href="/storefront"
                        className="inline-block px-4 py-2 rounded-[4px] bg-[#404d85] text-white font-bold text-xs hover:bg-[#323d6a] transition shadow-xs"
                      >
                        Explore Marketplace
                      </Link>
                    </div>
              ...
              <div className="rounded-[8px] border border-slate-200 bg-white p-5 sm:p-6 space-y-6 shadow-2xs">
                ...
                {wishlistItems.length === 0 ? (
                  <div className="p-8 text-center border border-dashed border-slate-200 rounded-[6px] bg-slate-50 space-y-3">
                    ...
                    <p className="text-xs text-slate-500 max-w-sm mx-auto">
                      Save your favorite products while browsing to keep track of prices, availability, and fast ordering.
                    </p>
                    <div className="pt-1">
                      <Link
                        href="/storefront"
              ...
              <div className="rounded-[8px] border border-slate-200 bg-white p-5 sm:p-6 space-y-6 shadow-2xs">
                ...
                {rmaReturns.length === 0 ? (
                  <div className="p-8 text-center border border-dashed border-slate-200 rounded-[6px] bg-slate-50 space-y-2">
                    <span className="text-3xl">🔄</span>
                    <h4 className="font-bold text-slate-800 text-sm">No return requests active</h4>
                    <p className="text-xs text-slate-500 max-w-sm mx-auto">
                      All your purchases are protected by Office Connect Escrow Guarantee. If an item needs return or repair, you can submit an RMA request anytime.
                    </p>
                  </div>
                ) : (
